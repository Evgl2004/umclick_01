...
## Подготовительная схема версий викторин
...
- После подготовки API и клиент работают по прежнему договору, `content_revision` не передаётся, версии не создаются обычными операциями, а защита использованной викторины не ослабляется.
...
- Подготовительная схема может быть структурно отменена и применена повторно только на пустых игровых таблицах в выделенной тестовой базе. Переключающие миграции и перенос прикладного поведения относятся к следующей части.
