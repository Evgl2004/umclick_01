...
- `widgets/quiz_preview_dialog.dart` — чистое локальное представление предпросмотра текущей формы без сетевых и игровых зависимостей.
...
«Предпросмотр» рядом с сохранением строит снимок непосредственно из текущих контроллеров через тот же `QuizDraftMapper.toPayload`, что и обычное сохранение. Поэтому новая и уже загруженная форма проходят общую нормализацию и содержательную проверку, включая пропуск необязательных пустых вариантов, но обработчик сохранения не вызывается. Ошибка оставляет весь ввод на месте и не открывает окно. Архивная форма и архивный режим сохраняют действующий запрет редактирования и предпросмотра.

Полноэкранный `QuizPreviewDialog` явно помечен как локальный режим. В нём вручную выбираются вопрос, экран аудитории или участника и состояние чтения, ответа или результата. Длинное описание доступно во внутренней области с ограниченной высотой и прокруткой, поэтому не вытесняет управление и вопрос; при ширине меньше 720 пикселей существующие прокручиваемые переключатели ограничиваются шириной окна. Представление применяет `question_only_on_display` и `show_choices_on_participant`, раскрывает правильность только в результате и снова скрывает её при обратном переключении. Для аудитории оно повторяет шесть цветов и значков экрана показа, для участника — четыре обозначения с тем же циклом, что в `ParticipantQuestionCard`. Оно не принимает ответы, не запускает таймер или игровой автомат, не обращается к `ApiClient`, WebSocket, странице показа или панели участника и не создаёт сессию, PIN, QR либо историческую версию. Закрытие диалога не заменяет контроллеры и не обновляет форму с сервера.

Widgets:

- `teacher_auth_card.dart` - регистрация/вход/профиль/выход.
- `teacher_quiz_builder_card.dart` - конструктор викторины.
- `quiz_preview_dialog.dart` — локальный предпросмотр несохранённой формы.
