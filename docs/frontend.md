...
Переключатель «Рабочие» / «Архив» управляет только соответствующим серверным списком и его выбранным идентификатором. Архивность фактически загруженной формы хранится отдельно: переход к рабочему списку не разблокирует ранее загруженную архивную карточку, а переход между списками и ошибка загрузки не очищают несохранённые значения и редакцию. В архивном списке форма временно доступна только для чтения; после возвращения рабочая форма снова редактируется. После восстановления карточку можно загрузить из рабочего списка и редактировать обычным способом. Создание сессии для архивного списка отключено. «Удалить навсегда» показывается только при серверном признаке `can_delete` и требует подтверждения, но окончательное решение всё равно принимает сервер. Успешное действие адресно убирает карточку из текущего списка. При `quiz_open_session_conflict`, `quiz_history_protected` или `quiz_archived` панель показывает локализованную причину и не сбрасывает выбранную карточку и несохранённую форму.
