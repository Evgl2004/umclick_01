...
- `Quiz` — викторина с обязательным владельцем; подготовительно содержит `archived_at` и `content_revision`, но текущий договор их ещё не использует, а после первой сессии всё содержимое по-прежнему неизменяемо.
- `QuizVersion` — подготовленная таблица будущего содержимого с внутренним номером, состоянием `draft/fixed`, полями викторины и ограничениями единственного черновика; обычные операции пока не создают её строки.
- `Question` — вопрос викторины; старая обязательная связь `quiz` действует, новая `quiz_version` пока необязательна и не используется.
- `Choice` - вариант ответа.
- `LiveSession` — сессия с создателем, публичным UUID `join_token` и PIN, уникальным среди незавершённых сессий; продолжает работать через обязательную `quiz`, а подготовленная `quiz_version` остаётся `NULL`.
...
Постоянная защита истории обеспечивается защищёнными связями, ограничениями PostgreSQL, проверками моделей/массовых операций и транзакциями. Завершение не разрешает редактирование или удаление использованного содержимого. Для [целевой модели версий и архивирования](quiz-versioning-and-archiving.md) реализована только подготовительная схема: она не меняет HTTP-договор, административный интерфейс, чтение истории или создание сессии. Семантическое переключение остаётся следующей отдельно разрешаемой частью. Точные действующие контракты и границы: [этап А](block-1-stage-a-api.md), [этап Б](block-1-stage-b-api.md) и [этап В](block-1-stage-v-api.md).
