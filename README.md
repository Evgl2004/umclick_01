...
- Общие Redis-квоты HTTP и WebSocket, ограниченные клиентские повторы и отказ `503` без локального обхода.
- Строковый UUID и раздельные `Bearer`/`Participant`/`Display` во всех клиентских сценариях, восстановление уже сохранённого участия и показа.
...
- Версии содержимого викторин, архивирование, защищённое удаление и локальный предпросмотр несохранённого черновика.
...
| [docs/block-1-stage-v-api.md](docs/block-1-stage-v-api.md) | Фактический контракт В: ошибки, квоты, повторы, ролевое хранение, WebSocket и Nginx |
| [docs/block-1-stage-v-handoff.md](docs/block-1-stage-v-handoff.md) | Передача локальной реализации В, проверки ПР-27–ПР-42 и непроверенные стендовые границы |
| [docs/quiz-versioning-and-archiving.md](docs/quiz-versioning-and-archiving.md) | Реализованная модель карточки, внутренних версий, предпросмотра, архивирования и сохранения игровой истории |
| [docs/quiz-versioning-and-archiving-analysis.md](docs/quiz-versioning-and-archiving-analysis.md) | Технический анализ, последовательность частей 1–5 и фактические доказательства реализации |
| [docs/quiz-versioning-and-archiving-handoff.md](docs/quiz-versioning-and-archiving-handoff.md) | Итоговый акт локальной реализации и однобазовой репетиции перехода; границы ещё не выполненного обновления сервера |
...
Любая связанная сессия навсегда запрещает изменение или удаление использованной версии викторины; история защищена от прямого и каскадного удаления. [Модель версий и архивирования](docs/quiz-versioning-and-archiving.md) реализована и проверена локально, включая предпросмотр и однобазовую репетицию перехода с адресным повтором полного сравнения содержимого. Это не является разрешением развёртывания или очистки рабочей базы.

После А преподаватель использует JWT и группу `teacher`, администратор — `is_staff`; каждое действие проверяет владельца. Серверный протокол проведения реализован в Б, а совместимый Flutter-клиент, общие ограничения и договор соединений — локально в В. Подробности: [контракт А](docs/block-1-stage-a-api.md), [контракт Б](docs/block-1-stage-b-api.md) и [контракт В](docs/block-1-stage-v-api.md).
